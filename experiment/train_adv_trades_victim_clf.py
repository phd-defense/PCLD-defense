...
def main_train_adv_trades_victim_clf(experiment: str, ds_local_path: str, device: str,
                                     batch_size: int, lr: float, n_epochs: int, epsilons: list):
    print('-' * NUM_OF_HYPHENS)
    print('Running train adv trades victim classifier experiment...')

    # define image transformations
    train_transform = transform_dataset(augmentations=True, to_integers=False)
    ...
    for epoch in range(1, n_epochs+1):
        ...
        # train-full
        results_df, results_deep_df = process_epoch_victim_adv_trades( experiment=experiment,
                                                                       epsilons=epsilons,
                                                                       device=device,
                                                                       epoch=epoch,
                                                                       net=net,
    ...
    print('Finished running train adv trades victim classifier experiment!')
